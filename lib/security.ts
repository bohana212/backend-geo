...
export function decryptSecret(payload: string): string {
  ...
  if (!ivB64 || !tagB64 || !encB64) throw new Error("Encrypted secret invalid.");
...
export function withCors(request: Request, response: Response): Response {
  ...
  headers.set("Access-Control-Allow-Headers", "Content-Type, x-api-key, x-admin-key, x-tracker-key");
  headers.set("Access-Control-Max-Age", "86400");
  return new Response(response.body, { status: response.status, headers });
}

/** CORS preflight response for tracker & send endpoints. */
